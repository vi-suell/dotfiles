...
--
-- navigate to vault
vim.keymap.set("n", "<leader>oo", ":cd ~/documents/ovaults/Zettelkasten/")

vim.keymap.set("n", "<leader>on", ":Obsidian New")
--
-- convert note to template and remove leading white space
vim.keymap.set("n", "<leader>ot", ":ObsidianTemplate Default Template<cr> :lua vim.cmd([[1,/^\\S/s/^\\n\\{1,}//]])<cr>")
-- strip date from note title and replace dashes with spaces
-- must have cursor on title
vim.keymap.set("n", "<leader>of", ":s/\\(# \\)[^_]*_/\\1/ | s/-/ /g<cr>")
--
-- search for files in full vault
vim.keymap.set("n", "<leader>os", ':Telescope find_files search_dirs={"~/documents/ovaults/Zettelkasten/"}<cr>')
--
vim.keymap.set("n", "<leader>oz", ':Telescope live_grep search_dirs={"~/documents/ovaults/Zettelkasten/"}<cr>')
--
...
vim.keymap.set("n", "<leader>ok", ":!mv '%:p' ~/documents/ovaults/Zettelkasten/6 - Full Notes/ <cr>:bd<cr>")
